...
const supportEmail = 'support@d-poms.diw.go.th'
...
const measurementTypeOptions = ['CEMS', 'BOD/COD Online', 'CEMS และ BOD/COD Online', 'ไม่เกี่ยวข้องกับการตรวจวัด']

function buildMailtoUrl(form) {
  const subject = form.subject || `แจ้งขอความช่วยเหลือ: ${form.topic || 'ทั่วไป'}`
  const body = [
    `อีเมล: ${form.email}`,
    `ชื่อ: ${form.name}`,
    `เบอร์ติดต่อ: ${form.phone}`,
    `หัวข้อช่วยเหลือ: ${form.topic}`,
    `ประเภทการตรวจวัด: ${form.measurementType}`,
    `เรื่อง: ${form.subject}`,
...
function SupportRequestPage() {
  ...
  const validateForm = () => {
    ...
    if (!form.topic) {
      nextErrors.topic = 'กรุณาเลือกหัวข้อ'
    }

    if (!form.measurementType) {
      nextErrors.measurementType = 'กรุณาเลือกประเภท'
    }
          ...
          >
            <Box sx={{ minWidth: 0, flex: 1 }}>
              ...
              <Typography variant="body2" color="text.secondary">
                กรอกแบบฟอร์มเพื่อส่งรายละเอียดให้เจ้าหน้าที่ทางอีเมล
              </Typography>
            </Box>
          </Stack>
        </Paper>
          ...
          <Stack spacing={3}>
            ...
            >
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  กรอกข้อมูลให้ครบถ้วนเพื่อเปิดอีเมลพร้อมรายละเอียดที่ต้องการส่ง
                </Typography>
              </Box>
              <Chip label={supportEmail} size="small" variant="outlined" color="primary" />
            </Stack>
            ...
            >
              ...
              <Box sx={{ gridColumn: { xs: '1 / -1', md: 'span 3' } }}>
                <LabeledField label="หัวข้อช่วยเหลือ">
                  <TextField
                    select
                    fullWidth
                    size="small"
              ...
              <Box sx={{ gridColumn: { xs: '1 / -1', md: 'span 3' } }}>
                <LabeledField label="ประเภทการตรวจวัด">
                  <TextField
                    select
                    fullWidth
                    size="small"
                    ...
                    helperText={errors.measurementType}
                    onChange={(event) => updateForm('measurementType', event.target.value)}
                  >
