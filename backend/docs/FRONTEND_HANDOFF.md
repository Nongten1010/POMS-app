...
หมายเหตุ: `regionalAccess` จะมีเฉพาะเจ้าหน้าที่/admin ที่ถูกกำหนดภาคไว้ ถ้ามีค่า backend จะกรองข้อมูลคำขอเชื่อมต่อและโรงงานตามภาคนั้นเพิ่มจาก permission scope เดิม
...
- `user.department`, `user.lineNameTh`, `user.levelNameTh`, `user.regionalAccess` ไม่บังคับ; ถ้าไม่ส่งหรือส่งค่าว่าง backend จะไม่เปลี่ยนค่าเดิม
- `user.regionalAccess.regions` ใช้ค่าภาค เช่น `ภาคตะวันออก`, `ภาคตะวันตก`, `ภาคตะวันออกเฉียงเหนือ`, `ภาคเหนือ`, `ภาคใต้`
- การส่ง/แก้ `regionalAccess` ต้องใช้ token ที่มี `permissions:manage`; token ที่มีเฉพาะ `users:edit` จะแก้ข้อมูล user ทั่วไปได้แต่แก้ภาคไม่ได้
