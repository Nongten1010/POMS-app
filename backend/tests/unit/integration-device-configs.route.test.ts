...
describe('integration device configs route', () => {
  ...
  it('accepts annual point codes supplied as an encoded path segment', async () => {
    const app = createApp();
    const stationId = 'WEMS-0003/2571';

    const response = await request(app)
      .get(`/api/v1/integrations/device-configs/${encodeURIComponent(stationId)}`)
      .set('X-API-Key', 'test-integration-key');
  ...
  it('rejects malformed point codes split into multiple path segments', async () => {
    const app = createApp();

    const response = await request(app)
      .get('/api/v1/integrations/device-configs/WEMS-0003/not-a-year')
      .set('X-API-Key', 'test-integration-key');
