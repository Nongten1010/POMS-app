...
const componentSchemas: Record<string, OpenApiObject> = {
  ...
  OperatorFactoryTableRow: {
    ...
    properties: {
      ...
      industryType: {
        type: 'string',
        nullable: true,
        description: 'คำอธิบายการประกอบกิจการจาก businessActivity; null เมื่อไม่มีข้อมูล',
      },
      industryMainOrder: {
        type: 'string',
        nullable: true,
        description: 'ลำดับหลักจากข้อมูลโรงงานเข้าข่าย; null เมื่อไม่มีข้อมูล',
      },
      industrySubOrder: {
        type: 'string',
        nullable: true,
        description: 'ลำดับย่อยจากข้อมูลโรงงานเข้าข่าย; null เมื่อไม่มีข้อมูล',
      },
      ...
      eiaOther: nullableString(
        500,
        'รายละเอียด EIA ที่บันทึกไว้; connection factory lists คืนให้โรงงานเข้าข่ายทั้งโหมดข้อมูลเดิมและ canonical; optional nullable',
      ),
      projectName: {
        type: 'string',
        nullable: true,
        description: 'ชื่อโครงการที่บันทึกไว้; null เมื่อไม่มีข้อมูล',
      },
  ...
  ConnectionRequestTableRow: {
    ...
    required: [
      ...
      'codeIssuedDate',
      'connectionDueAt',
      'waitingConnectionDaysRemaining',
    ...
    properties: {
      ...
      province: { type: 'string', nullable: true },
      type: { type: 'string', enum: ['CEMS', 'WPMS'] },
      requestNo: { type: 'string' },
      submittedAt: { type: 'string', format: 'date-time' },
      submittedDate: { type: 'string', description: 'วันที่แบบ DD/MM/พ.ศ.' },
      monitoringPointCode: {
        type: 'string',
        nullable: true,
        example: 'S2001',
        description:
          'รหัสจุดตรวจวัดจาก measurementPoints[0].pointCode ของคำขอ; null เมื่อยังไม่มีรหัสหรือไม่มีจุด',
      },
      monitoringPointName: {
        type: 'string',
        nullable: true,
        example: 'ปล่องระบาย A',
        description:
          'ชื่อจุดตรวจวัดจาก measurementPoints[0].pointName ใน snapshot คำขอ เป็นจุดเดียวกับ monitoringPointCode; null เมื่อไม่มีจุด และยังคืนชื่อแม้ยังไม่มีรหัส แสดงคอลัมน์ถัดจากรหัสจุดตรวจวัด',
      },
      codeIssuedAt: { type: 'string', format: 'date-time', nullable: true },
      codeIssuedDate: { type: 'string', nullable: true },
      connectionDueAt: { type: 'string', format: 'date-time', nullable: true },
      waitingConnectionDaysRemaining: { type: 'integer', nullable: true },
      waitingConnectionText: { type: 'string', nullable: true },
      form: { type: 'string' },
      status: { type: 'string' },
      statusCode: { type: 'string', enum: Object.keys(CONNECTION_REQUEST_STATUS_LABELS) },
      requestType: { type: 'string', enum: Object.keys(CONNECTION_REQUEST_TYPE_LABELS) },
    },
  },
  ConnectionRequestTableResponse: {
    type: 'object',
    required: ['success', 'data', 'meta'],
    properties: {
      success: { type: 'boolean', enum: [true] },
      data: { type: 'array', items: schemaRef('ConnectionRequestTableRow') },
      meta: {
        type: 'object',
        required: ['total'],
...
const connectionRequestPaths: Record<string, OpenApiObject> = {
  ...
  '/cems-wpms-requests/table-rows': {
    get: securedOperation({
      ...
      successExamples: {
        requestTable: {
          summary: 'รายการคำขอพร้อมรหัสและชื่อจุดตรวจวัด',
          value: {
            success: true,
            data: [
              {
                ...
                form: CONNECTION_REQUEST_TYPE_LABELS.NEW_CONNECTION,
                status: CONNECTION_REQUEST_STATUS_LABELS.CONNECTED,
