...
const pomsFactoryEditRequestSchema: OpenApiObject = {
  ...
  properties: {
    ...
    currentMeasurementPoints: {
      ...
      description:
        'GET edit-requests/{id}: snapshot ก่อนแก้เฉพาะ connectedPointId ที่ทำคำขอ ตามกติกาจุดเป้าหมายเดียวกับ list; คงลำดับ snapshot เดิม. คำขอเก่าใช้ SNAPSHOT_DIFF ก่อน derive กลุ่มพารามิเตอร์; UNKNOWN คืน [] เมื่อมี snapshot และคง null เมื่อไม่มี snapshot. BASIC_INFO คงเดิม. การกรองนี้ไม่เปลี่ยน snapshot ที่บันทึกไว้หรือ response ของ create/resubmission/review/cancel',
    },
    proposedMeasurementPoints: {
      type: 'array',
      nullable: true,
      items: schemaRef('PomsMeasurementPoint'),
      description:
        'GET edit-requests/{id}: snapshot หลังแก้เฉพาะ connectedPointId ชุดเดียวกับ currentMeasurementPoints รวมจุดที่เลือกแต่ค่าไม่เปลี่ยนเมื่อมี SUBMITTED IDs; ไม่กรองเพียง systemType. คงลำดับ snapshot เดิมและรายละเอียดที่เสนอ. UNKNOWN คืน [] เมื่อมี snapshot และคง null เมื่อไม่มี snapshot. response ของ create/resubmission/review/cancel คง snapshot เต็มตามเดิม',
    },
...
const componentSchemas: Record<string, OpenApiObject> = {
  ...
  PomsFactoryMeasurementPointPatchRequest: {
    ...
    properties: {
      officerNotificationEmails: {
        ...
        items: { type: 'string', format: 'email', maxLength: 254 },
        description:
          'อีเมลเจ้าหน้าที่ของจุดนี้; trim, lowercase และตัดซ้ำ. ไม่ส่ง = คงเดิม, [] = ล้างรายชื่อ, null ไม่ได้. ใช้ค่าใหม่เมื่อคำขออนุมัติแล้ว',
      },
  ...
  PomsFactoryEditRequestDetail: {
    ...
    required: [
      ...(pomsFactoryEditRequestSchema.required as string[]),
      'contactPersons',
      'notificationEmails',
      'officerNotificationEmails',
    ...
    properties: {
      ...(pomsFactoryEditRequestSchema.properties as OpenApiObject),
      contactPersons: {
        type: 'array',
        maxItems: 20,
        items: schemaRef('ContactPerson'),
      ...
      informationProviderName: {
        ...
        description:
          'ชื่อผู้ให้ข้อมูลหรือผู้รับมอบอำนาจจาก source connection request; null เมื่อไม่มีค่า',
      },
      informationProviderPosition: {
        ...nullableStringSchema(255),
        description:
          'ตำแหน่งผู้ให้ข้อมูลจาก source connection request เดียวกับชื่อ; null เมื่อไม่มีค่า',
      },
    },
  },
  ...
  PomsEditRequestTargetPoint: {
    type: 'object',
    additionalProperties: false,
    required: ['connectedPointId', 'systemType', 'pointCode', 'pointName'],
    properties: {
      connectedPointId: { type: 'integer', minimum: 1 },
    ...
    properties: {
      ...Object.fromEntries(
        [
          ...
          key,
          (pomsFactoryEditRequestSchema.properties as Record<string, unknown>)[key],
        ]),
      ),
      provinceName: {
        ...nullableStringSchema(128),
        description: 'จังหวัดจาก active eligible metadata; null เมื่อไม่มีข้อมูล',
      },
      targetMeasurementPoints: {
        type: 'array',
        items: schemaRef('PomsEditRequestTargetPoint'),
        description:
          'จุดจาก ID ที่ส่งใน create/resubmission ล่าสุด ไม่ใช่ทุกจุดของโรงงาน; BASIC_INFO หรือไม่ทราบเป้าหมายคืน []',
      },
      targetMeasurementPointsSource: {
        type: 'string',
        enum: ['SUBMITTED', 'SNAPSHOT_DIFF', 'UNKNOWN', 'NOT_APPLICABLE'],
        description:
          'SUBMITTED = ID ที่บันทึกจาก payload ล่าสุด; SNAPSHOT_DIFF = อนุมานจุดที่เปลี่ยนจาก snapshot เก่า อาจไม่ครบจุดที่ส่งค่าเดิม; UNKNOWN = หลักฐานไม่พอ/ไม่สมบูรณ์ ห้ามเลือกจุดแรกแทน; NOT_APPLICABLE = BASIC_INFO',
      },
    },
    example: {
      id: 48,
  ...
  PomsFactoryEditRequestsResponse: {
    ...
    properties: {
      success: { type: 'boolean', enum: [true] },
      data: { type: 'array', items: schemaRef('PomsFactoryEditRequestSummary') },
      meta: {
        type: 'object',
        additionalProperties: false,
        required: ['total'],
  ...
  BodCodWorkflowStep: {
    ...
    properties: {
      ...
      roleCode: { type: 'string', enum: ['INSPECTOR', 'RESULT_NOTICE', 'REVIEWER', 'APPROVER'] },
      roleLabel: { type: 'string' },
      status: {
        type: 'string',
        enum: ['PENDING', 'WAITING', 'APPROVED', 'REJECTED', 'REVISION_REQUESTED'],
      },
      actorUserId: { type: 'integer', minimum: 1, nullable: true },
      actorName: {
        type: 'string',
        nullable: true,
        description:
          'ชื่อผู้ดำเนินการของ step: ใช้ actor_name ที่บันทึกไว้ก่อน หากว่างใช้คำนำหน้า ชื่อ และนามสกุลจาก users ตาม actorUserId ณ เวลาอ่าน โดย trim แต่ละส่วน; หากไม่มีชื่อใช้ username และคืน null เมื่อไม่มีผู้ดำเนินการหรือไม่พบชื่อ ชื่อที่อ่านจาก users อาจเปลี่ยนตามข้อมูลผู้ใช้',
        example: 'นาย สมชาย ใจดี',
      },
      actorPosition: { type: 'string', nullable: true },
  ...
  BodCodReportData: {
    ...
    properties: {
      ...
      steps: {
        type: 'array',
        items: schemaRef('BodCodWorkflowStep'),
        description: 'ขั้นตอนของรายงานเรียงตาม stepNo พร้อม actorName ของผู้ดำเนินการแต่ละขั้นตอน',
      },
      allowedActions: {
        type: 'array',
...
const extraPaths: Record<string, OpenApiObject> = {
  ...
  '/poms-factories/edit-requests': {
    get: securedOperation({
      ...
      description:
        'คืนข้อมูลสรุปเฉพาะคำขอของโรงงานใน data scope ของ factories:view พร้อม targetMeasurementPoints และ targetMeasurementPointsSource ตามหลักฐานคำขอล่าสุด; ไม่คืน snapshots, contacts, documents หรือ events ใน list ให้เรียก GET /poms-factories/edit-requests/{id} เมื่อต้องการรายละเอียด; คง filter และเรียง createdAt/id ใหม่ก่อน',
      parameters: [
  ...
  '/bod-cod-deviation-reports/{id}': {
    get: securedOperation({
      ...
      description:
        'ต้องมี bod_cod_errors:view และรายงานอยู่ใน data scope ของผู้ใช้ คืน steps[].actorName เป็นชื่อผู้ดำเนินการแต่ละขั้นตอน (string หรือ null); currentStep ใช้ข้อมูลขั้นตอนเดียวกับ steps',
      parameters: [idParameter],
      successSchema: schemaRef('BodCodReportResponse'),
    }),
  },
