...
ควรเพิ่ม `accountType` ใน request เพื่อป้องกันกรณี username เดียวกันมีทั้งบัญชี POMS และ API.
...
1. `accountType = poms` ต้องตรวจเฉพาะ local credential และห้าม fallback ไป API.
2. `accountType = api` ต้องตรวจเฉพาะ external API และห้าม fallback ไป local account.
3. ภายใน `api` ให้ router แยก `diw_dpis` กับ `i_industry` จากรูปแบบ login identifier ที่ผ่าน validation.
4. กติกา username DPIS ที่สมบูรณ์ต้องยืนยันกับเจ้าของระบบก่อนบังคับ regex; requirement ที่ทราบตอนนี้คือบัญชีลักษณะ `U...` และเลข 13 หลัก.
5. ระหว่าง compatibility window อาจยังรับ legacy `provider: "local"` ได้ แต่ frontend ใหม่ต้องส่ง `accountType` เสมอ และ backend ต้องประกาศวันเลิกรับ field เดิม.

ห้ามใช้ local-first fallback เมื่อ request ส่ง `accountType` มาแล้ว เพราะจะทำให้บัญชีคนละแหล่งเข้าสู่ user ผิดตัวได้.
...
- login ใหม่ที่ระบุ `accountType=api` ต้อง provision/find account ด้วย provider ใหม่และ login identifier ที่ผู้ใช้กรอก.
...
- เพิ่ม `accountType` ใน login validator โดยยังรับ request เก่าชั่วคราว.
- เมื่อ request มี `accountType` ต้องห้าม fallback ข้าม `poms`/`api`.
...
- request เก่าที่ยังไม่มี accountType อาจใช้ legacy flow เฉพาะ compatibility window; request ใหม่ห้าม fallback ข้าม account type/provider.
...
| --- | --- | --- |
| Request validation | `backend/src/modules/auth/auth.validator.ts` | เพิ่ม `accountType`; validate POMS/API request แบบแยกกัน |
| Auth DTO | `backend/src/modules/auth/auth.types.ts` | เพิ่ม target user contract และ compatibility fields |
| Login orchestration | `backend/src/modules/auth/auth.service.ts` | ตัด local-first เมื่อ accountType ชัดเจน; สร้าง response pattern ใหม่ |
...
- เพิ่มวิธีเลือก/ระบุ `accountType = poms | api` สำหรับ officer login.
- ส่ง `departmentID` เฉพาะ API officer flow ที่ต้องใช้.
- ห้าม frontend ส่งชื่อ internal provider เช่น `diw_dpis` หรือ `i_industry`.
- request เก่าที่ไม่มี `accountType` ใช้ได้เฉพาะ compatibility window.
...
- อัปเดต login request ให้มี `accountType`.
...
| --- | --- |
| `POST /api/v1/auth/login` | request มี accountType; target auth response |
...
- [x] local กับ API เลือก account ด้วย `accountType` และ local lookup เป็น provider-scoped.
...
- [ ] เปลี่ยน `accountType` ใน request ไม่ทำให้ bypass password หรือ fallback ข้าม provider.
...
5. Deploy frontend ที่ส่ง accountType และอ่าน target response.
6. Monitor login success/failure แยก provider ด้วย aggregate metrics ที่ไม่มี PII.
7. บังคับ accountType และหยุด legacy fallback หลัง client ทุกตัวพร้อม.
...
- [ ] Frontend ส่ง `accountType` และไม่รู้ internal provider.
