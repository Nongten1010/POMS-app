...
| `industrySubOrder` / `factorySubclass` | `FACCLASS.CLASS` join ด้วย `FID` | ใช้เลข 3 หลักท้ายจาก `FACCLASS.CLASS`, ตัดค่าซ้ำและตัดรหัสที่ซ้ำกับ `industryMainOrder`, มากกว่า 1 ค่า join ด้วย comma; ถ้าไม่เหลือค่าให้คืน `null` |
