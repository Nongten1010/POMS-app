...
| `industryMainOrder` / `factoryClass` | `fac_import.CLASS` | มีถ้า column `CLASS` มีใน source; ตาม DataDict อาจต้อง join `FAC_CLASS.CLASS` ด้วย `FID` |
| `industrySubOrder` / `factorySubclass` | `fac_import.SUBCLASS`, fallback `FACTYPE`/`FACREQ`/`EXPSEQ` | มีถ้า column เหล่านี้มีใน source; ตาม DataDict อาจต้อง join `FAC_CLASS.DISP_CLASS` ด้วย `FID` |
...
| `eia` / `hasEia` | ไม่มีใน `fac_import` mapping ปัจจุบัน | ต้องหา source จากระบบ/ตาราง EIA หรือให้เจ้าหน้าที่กรอกตอนเลือกโรงงาน |
| `projectName` | ไม่มีใน `fac_import` mapping ปัจจุบัน | ต้องหา source จากระบบ/ตาราง EIA/project หรือให้ผู้ประกอบการกรอกในฟอร์ม |
