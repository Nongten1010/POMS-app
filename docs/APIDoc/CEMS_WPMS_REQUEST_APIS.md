...
| `industrySubOrder` / `factorySubclass` | `FACCLASS.CLASS` join ด้วย `FID`, fallback `fac_import.CLASS` | ใช้เลข 3 หลักท้ายจาก `FACCLASS.CLASS`, ตัดค่าซ้ำ, มากกว่า 1 ค่า join ด้วย comma; ถ้า `FACCLASS` ไม่มีค่าให้ fallback จาก `fac_import.CLASS` |
