...
| `industrySubOrder` / `factorySubclass` | `fac_import.DISPFACREG`, fallback `FACREG`, fallback `FACCLASS.CLASS`, fallback `fac_import.CLASS` | ใช้เลขลำดับหลังประเภทโรงงานจากทะเบียน เช่น `3-1-2/17ลป` -> `002`; ถ้าทะเบียนอ่านไม่ได้จึง fallback ไป `FACCLASS`/`CLASS` |
