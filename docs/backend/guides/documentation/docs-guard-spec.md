...
- Base และ head SHA หรือ commit range ของ change ที่กำลังส่ง
- Pull-request body หรือ direct-push change summary สำหรับ documentation-impact block
...
เมื่อ diff แตะ `backend/` ต้อง parse fields ตาม [documentation-impact template](./pr-documentation-impact-template.md) จาก PR body หรือ change summary ของ direct push:
...
- รันใน delivery path ที่ใช้งานจริง: pull-request workflow เมื่อใช้ PR และ push validation เมื่อผู้ใช้สั่ง direct push
- transition-mode fixtures ผ่านทั้งหมด
- ผลตรวจ deterministic ระหว่าง local และ CI
- branch protection เป็น optional hardening ไม่ใช่เงื่อนไขบังคับ; หากผู้ใช้เปิดใช้ ให้ตั้ง guard เป็น required check
