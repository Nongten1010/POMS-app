...
การส่งแบบแก้ไขลบจุดเก่าที่ไม่ได้เชื่อมต่อออกถาวร รวมรายการเก่าที่มี `deleted_at` และคืนเฉพาะรหัสที่ทะเบียนระบุเจ้าของตรงกับจุดและคำขอนั้น การลบและคืนรหัสอยู่ใน transaction เดียวกัน เมื่อมีการอ้างอิงที่ทำให้คืนรหัสไม่ได้ ทั้งคำขอจะไม่เปลี่ยนแปลง

ตัวคำขอและ `statusHistory` ยังอยู่ ส่วนจุดใหม่ได้รับ ID ใหม่ รหัส legacy ที่คืนสำเร็จสามารถกำหนดใหม่ด้วย `MANUAL_LEGACY` ในขั้นอนุมัติ หากยังว่างและผ่านกฎช่วง `S/P0001–1999` และ `systemType` เดิม การยื่น `ADD_PARAMETER` ยังคงการจองของจุดต้นทางที่เชื่อมต่ออยู่
...
[repository regression](../../../backend/tests/unit/connection-requests.current-factory-profile.repository.test.ts) ใช้ฐานข้อมูลจำลองที่มี state ทดสอบ `replaceForm → updateStatus` ด้วยรหัส `S0527`, การคงการจองของเจ้าของอื่น และ rollback เมื่อคืนรหัสไม่ได้ ชุดทดสอบนี้ไม่ใช่การยืนยันว่า migration trigger รันสำเร็จบน SQL Server จริง
