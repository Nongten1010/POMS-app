...
ตัวอย่างผลลัพธ์: เดิมมี `Flow Rate (m3/hr)` แล้วขอเพิ่ม `NOx (ppm)`, `SO2 (ppm)`, `O2 (%)` เมื่อเชื่อมต่อสำเร็จ รายการจุดปัจจุบันและ [ฟอร์มเพิ่มพารามิเตอร์](./README.md#add-parameter-prefill) ต้องแสดงครบทั้ง 4 ตัว พร้อมข้อมูลเครื่องมือเดิม ประวัติคำขอและ response ของคำขอยังคงเป็นข้อมูลที่ยื่นในคำขอนั้น
...
- ถ้าสถานะสุดท้ายเป็น `CONNECTED` และ `details.pendingParameters` มีพารามิเตอร์จริงอย่างน้อย 1 ค่า ต้องมีพารามิเตอร์ที่เลือกอย่างน้อย 1 ค่า โดย backend ใช้ `details.requestedParameters` เป็นค่าหลักเมื่อส่ง field นี้ แล้วจึง fallback ไป `measurementInstruments.parameters` หรือ `parameters`; ถ้า resolve ไม่ได้ backend ปฏิเสธคำขอเพื่อไม่ให้เปิดจุดที่ไม่มี parameter snapshot สำหรับหน้าตั้งค่าอุปกรณ์
...
| HTTP status | Code               | Condition                                                                                                                                                     | Client action                    |
| ----------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| `400`       | `VALIDATION_ERROR` | ไม่มี `factoryId/factoryRegistrationNo`, action/status ขัดกัน, ขาด `revisionReason`, มีหลาย measurement points, `CONNECT` ทั้งที่มี pending parameter แต่ไม่มีพารามิเตอร์ที่เลือก, document row ไม่ครบ, หรือ body ไม่ผ่าน schema | แก้ payload และส่งใหม่           |
| `401`       | `UNAUTHORIZED`     | ไม่มี token หรือ token ใช้ไม่ได้                                                                                                                              | login ใหม่                       |
