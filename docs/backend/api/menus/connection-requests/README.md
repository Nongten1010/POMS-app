...
ข้อยกเว้นสำหรับ active point ที่ได้รับการยกเว้นทั้งหมด: ผู้ประกอบการส่ง `measurementPoints[].monitoringPointStatus = "ได้รับการยกเว้นทั้งหมด"` พร้อมรายการพารามิเตอร์ว่าง เมื่อเจ้าหน้าที่อนุมัติแบบ backend ออกรหัสจุดและเปลี่ยนคำขอเป็น `CONNECTED` พร้อมสร้าง current/live active point ทันที โดยไม่ผ่านขั้นตั้งค่าอุปกรณ์, confirm และ verify. จุดนี้ยังใช้ยื่นแบบ กวภ. และใช้ `pointCode` เดิมทำคำขอ `ADD_PARAMETER` เมื่อพร้อมติดตั้งในภายหลัง.
...
| upload รูป/เอกสารสำหรับฟอร์ม                  | `POST` | `/api/v1/cems-wpms-requests/document-images`              | `multipart/form-data`          | `cems_wpms_requests:edit`           | ต้องมี `file` หรือ `link`; รูปจุดระบายน้ำทิ้ง WPMS ใช้ `title=ภาพถ่ายจุดระบายน้ำทิ้งออกนอกโรงงาน`; ไฟล์ JPEG/PNG/PDF สูงสุด 5 MiB |
...
- คำขอ `ADD_PARAMETER` ใช้รหัสจุดเดิมและไม่ออกรหัสใหม่ โดยต้องอ้าง active connected point ที่มี `eligible_factory_id` และ `systemType` ตรงคำขอ ตาม [กติกาเจ้าของจุดสำหรับเพิ่มพารามิเตอร์](./request-payloads-and-validation.md#post-apiv1cems-wpms-requestsparameters).
...
| `measurementPoints[0].pointCode` | string           | yes         | trim แล้วต้องไม่ว่าง, ยาวไม่เกิน 64 ตัวอักษร และห้ามซ้ำกับรหัสที่ยังจองอยู่ในทะเบียนรหัสกลาง                               |
...
- โรงงานเข้าข่ายคืน `eligibilityRequest: null` และ `canRequestEligibility: false`
...
| `data[].eligibilityRequest`                           | object \| null                  | คำขอเพิ่มโรงงานที่ยัง `PENDING_REVIEW`; เป็น `null` เมื่อเข้าข่ายแล้วหรือไม่มีคำขอค้าง                       |
| `data[].eligibilityRequest.id`                        | number                          | request id สำหรับอ้างอิงคำขอค้าง                                                                           |
| `data[].eligibilityRequest.status`                    | `"PENDING_REVIEW"`              | machine status ของคำขอค้าง                                                                                  |
...
| `data.status`                                      | string         | ปกติเป็น `WAITING_CONNECTION`; เป็น `CONNECTED` เมื่อทุกจุดได้รับการยกเว้นทั้งหมด |
...
สำหรับจุดที่ `monitoringPointStatus = "ได้รับการยกเว้นทั้งหมด"` response หลังอนุมัติคืน `status: "CONNECTED"`, `verifiedAt` เป็นเวลาอนุมัติ, `measurementPoints[].parameters: []` และรหัสจุดที่ออกให้ตามกติกาเดิม. Active point ถูกบันทึกใน `cems_wpms_connected_measurement_points` พร้อม `monitoring_point_status` เพื่อให้ query และนับสถานะปัจจุบันได้โดยไม่อ่านประวัติคำขอ.
...
- `409 CONFLICT` พร้อม `error.details.reason = "ADD_PARAMETER_POINT_OWNERSHIP_INVALID"` และ `path = "measurementPoints.i.pointCode"` เมื่อคำขอเพิ่มพารามิเตอร์อ้างจุดที่ไม่ผ่าน [กฎเจ้าของโรงงานและระบบ](./request-payloads-and-validation.md#post-apiv1cems-wpms-requestsparameters) ใช้กับ `POST /parameters`, `PUT /:id/form`, การอนุมัติผ่าน `POST /:id/review` หรือ `POST /:id/status`, `POST /:id/confirm-connection` และ `POST /:id/verify-connection`; ไม่มีการบันทึกบางส่วนหรือเปิดเผยรายละเอียดจุดของผู้อื่น
...
| Operator eligibility request state | [`connection-requests.repository.ts`](../../../../../backend/src/modules/connection-requests/connection-requests.repository.ts), [`connection-requests.service.ts`](../../../../../backend/src/modules/connection-requests/connection-requests.service.ts), [`connection-requests.operator-factories.repository.test.ts`](../../../../../backend/tests/unit/connection-requests.operator-factories.repository.test.ts), [`connection-requests.operator-factories.route.test.ts`](../../../../../backend/tests/unit/connection-requests.operator-factories.route.test.ts), [TDD evidence](../../../evidence/eligible-factories/eligible-factory-requests.tdd.md) |
...
การตรวจเจ้าของจุด `ADD_PARAMETER`: [repository](../../../../../backend/src/modules/connection-requests/connection-requests.repository.ts), [regression การตรวจเจ้าของและ transaction](../../../../../backend/tests/unit/connection-requests.current-factory-profile.repository.test.ts) และ [runtime OpenAPI contract tests](../../../../../backend/tests/unit/connection-requests.parameter-ownership.openapi.test.ts).
