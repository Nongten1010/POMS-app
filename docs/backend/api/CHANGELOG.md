...
- **Affected canonical docs:** [กวภ.01–05](./menus/kwp-forms/README.md#kwp-handoff-fields)
- **Impact:** เพิ่มไฟล์/ลิงก์ กวภ.01/03 รอบ/ปีและลิงก์ กวภ.02/04 การยกเลิกโดยผู้ประกอบการ และ API prefill พารามิเตอร์ที่เข้าข่ายตามสิทธิ์ กวภ. การสร้าง/แก้ไขตรวจพารามิเตอร์ที่เข้าข่ายทั้งหมดและสิทธิ์พื้นที่ ไฟล์ใหม่ต้องเป็นไฟล์อัปโหลดของบัญชีผู้ส่งและ metadata ตรงกับไฟล์จริง ชุดทั่วไปจำกัด 5 ไฟล์; ชุดภาพ/ผลห้องปฏิบัติการจำกัดชนิดละ 5 ไฟล์รวมทุกแถว
- **Migration:** รัน migration `0121_add_kwp_submission_attachments_and_report_period.ts` ก่อนเปิด backend เปลี่ยน frontend prefill เป็น `GET /kwp-form-reports/factories/:factoryId/measurement-points` และโหลดรายการพารามิเตอร์ใหม่ ส่ง metadata ที่ upload API คืนจริง ไฟล์ที่อัปโหลดค้างก่อนรุ่นนี้และยังไม่เคยผูกคำขอต้องอัปโหลดใหม่ ไฟล์เดิมที่ผูกคำขอแล้วเก็บต่อได้โดยไม่เปลี่ยน metadata; ไม่มีการเติมปีปัจจุบันย้อนหลัง
- **Breaking change:** yes — ปฏิเสธพารามิเตอร์นอกชุดที่เข้าข่าย จำนวนไฟล์เกินเพดานใหม่ และไฟล์ที่ไม่ได้อัปโหลดโดยบัญชีผู้ส่งหรือยังไม่เคยผูกคำขอที่แก้ไข หลัง deploy ต้องตรวจ runtime OpenAPI และทดสอบ flow กับ SQL Server
...
## รายการโรงงานรองรับสถานะ Admin

`GET /api/v1/poms-factories` เปลี่ยน `status` จากค่าคงที่ `แสดง` เป็น `แสดง`, `ซ่อน` หรือ `ยกเลิกการเชื่อมต่อ` ตามค่าที่ Admin บันทึก ผู้เรียกที่สมมติว่า status มีค่าเดียวต้องรองรับ enum ทั้งสาม ค่าเริ่มต้นของข้อมูลเดิมยังเป็น `แสดง` ดู [สัญญาจัดการสถานะ](./menus/master-data/status-management.md) และใช้ GET/PATCH status-management สำหรับหน้าต่างจัดการสถานะ
