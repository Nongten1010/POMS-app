...
| Regression         | targeted request-number, request-flow และ migration suites 9 ไฟล์                                                                                           | PASS: 9 suites, 197 tests    | `requestNo` ของ WPMS เป็น `WPMS-...`; sequence query ของ WPMS ยังนับ `WEMS-...` ระหว่าง rollout; migration backfill ตรวจ collision และกัน legacy writer ก่อนจบ migration |
...
| 5   | Migration `0094_backfill_wpms_request_number_prefix` แปลงเฉพาะ request rows ของ WPMS ที่ยังเป็น `WEMS-NNNN/YYYY`, หยุดเมื่อชนเลขปลายทาง และกัน legacy writer หลัง backfill | `wpms-request-number-migration.test.ts`                                                                           | Migration unit     | PASS   |
| 6   | `requestNo` แยกจาก sequence ของ `measurementPoints[].pointCode`                                                                                                            | repository tests + point-code contract แยก                                                                        | Scope verification | PASS   |

## Coverage and Known Gaps

Focused coverage เดิมผ่าน 2 suites / 38 tests และ regression ล่าสุดเติม targeted suites ของ repository/route/service/migration สำหรับกรณี `WEMS -> WPMS` ให้ชัดขึ้น. การตั้งค่า Jest เก็บ coverage ทั้ง backend จึงรายงานรวมต่ำกว่าขอบเขตงานย่อยนี้; จุดที่แก้จริงถูก execute ทั้งใน request-number formatter, WPMS query scope, route/service response และ migration unit.

การเปลี่ยนนี้ยังใช้ `COUNT + 1` ตามพฤติกรรม allocator เดิม แต่ backend ของ `systemType = WPMS` จะนับทั้ง `WPMS-%/BBBB` และ `WEMS-%/BBBB` ระหว่าง rollout เพื่อหลีกเลี่ยงเลขซ้ำหากมีข้อมูลเดิมค้างก่อน migration. Migration เพิ่ม check constraint หลัง backfill เพื่อป้องกัน backend รุ่นเก่าสร้าง annual `WEMS-...` ในหน้าต่าง deploy. ข้อมูลย้อนหลังถูกแก้ผ่าน `0094_backfill_wpms_request_number_prefix` และจำกัดเฉพาะ `cems_wpms_connection_requests.request_no`.
