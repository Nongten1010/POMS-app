...
| Initial BOD/COD workflow steps followed the earlier design | Repository test was added before exporting the helper | Targeted repository test passed | Superseded by `docs/testing/bod-cod-result-notice-workflow.tdd.md`, which adds the result-notice editing step before review/approval |
...
| 9 | Historical approval step definitions were covered by the original API tests; current workflow shape is covered in `bod-cod-result-notice-workflow.tdd.md` | `backend/tests/unit/bod-cod-deviation-reports.repository.test.ts` | repository | PASS | `npm test -- --runTestsByPath ...` |
