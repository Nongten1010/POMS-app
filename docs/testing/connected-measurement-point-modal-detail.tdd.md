...
As an officer viewing a factory's connected measurement points, I want a narrow factory-scoped detail endpoint, so that the modal table can display point code, point name, point type, parameter details, primary fuel, and secondary fuel for every connected point in that factory.
...
| RED/GREEN fuel fields | Added service test coverage requiring `primaryFuel` and `secondaryFuel` from point `details`, then mapped those fields into the modal DTO. | `npm test -- --runInBand tests/unit/connection-requests.service.test.ts -t "returns modal detail rows"` | RED failed because fuel fields were missing; GREEN passed after adding the mapper |
...
| 4 | The response contains only `success`, `data`, and `meta`; each `data[]` row contains `pointCode`, `pointName`, `pointType`, `parameterDetails`, `primaryFuel`, and `secondaryFuel`. | `backend/tests/unit/connected-measurement-points.route.test.ts`; `backend/tests/unit/connection-requests.service.test.ts` | route/unit + service/unit | PASS | Exact body equality assertion |
| 5 | Fuel field names follow existing form names and blank fuel values return `null`. | `backend/tests/unit/connection-requests.service.test.ts` | service/unit | PASS | `details.primaryFuel` maps to `primaryFuel`, `details.secondaryFuel` maps to `secondaryFuel`, and blank secondary fuel maps to `null` |
