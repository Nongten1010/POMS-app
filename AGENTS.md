...
- Follow the Git delivery method explicitly requested by the user. Direct push to `main` is allowed only when the user explicitly requests it and the exact commit range, changed files, and relevant checks have been reviewed first. Otherwise prefer a feature branch and pull request for changes that need review. Do not treat the documentation workflow's PR recommendation as a hard prohibition unless branch protection is actually enabled by the user.
...
- Every API addition or client-visible API change must update the runtime OpenAPI contract served by `https://d-poms.diw.go.th/api/v1/docs/` in the same change. Maintain the source under `backend/src/modules/api-docs/`, including request and response schemas, permissions, statuses, validation rules, and examples; keep it aligned with the canonical page under `docs/backend/api/`. After a production deployment, verify `https://d-poms.diw.go.th/api/v1/openapi.json` exposes the same contract before considering the release complete.
